...
// Cache dynamique pour faire correspondre LID et Numéro de téléphone
...
const decodeJid = (jid) => {
    if (!jid) return jid;
    if (jid.includes("@lid") || /:\d+@/gi.test(jid)) {
...
async function messages_upsert(m, menma) {
    ...
    const selfJid = decodeJid(menma.user.id);
    if (ms.key.fromMe && !verif_Gp) {
        // Redirection vers le Bot JID UNIQUEMENT si on se parle à soi-même.
        // On compare le remoteJid avec notre numéro propre.
        if (ms_org === id_Bot || ms_org === selfJid || ms_org === "status@broadcast" || ms_org === "") {
            ms_org = id_Bot;
        }
    }
    ...
    // Mettre à jour le cache LID dynamiquement
    if (auteur_raw && auteur_raw.endsWith("@lid") && auteur_num) {
        lidCache.set(auteur_lid_num, auteur_num);
    }
    // Alimenter le cache avec tous les participants du groupe si on les a
    raw_participants.forEach(p => {
        const p_lid = (p.lid || p.lidJid || "").split("@")[0].replace(/\D/g, "");
        const p_num = getNumber(p.id);
        if (p_lid && p_num) lidCache.set(p_lid, p_num);
    });
    ...
    // ================================================================
    const is_Owner = ms.key.fromMe                  // Toujours true si c'est nous qui envoyons
        || auteur_num === bare_owner                 // Match numéro téléphone (224...)
        || auteur_num === bare_bot                   // Match numéro bot
        || (bare_lid && (auteur_lid_num === bare_lid || auteur_num === bare_lid))
        || (lidCache.get(auteur_lid_num) === bare_owner); // Détection dynamique via cache LID
